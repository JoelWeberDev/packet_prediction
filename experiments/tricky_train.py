...
@Author: Joel Weber
@Date: 2025-07-22
@Description: Rather than trying to climb over the wall we will run directly through it. Since we
have highly repetetive data the thing to do is actually train a micro model on the fly. This micro
model will be some recurrent model such as a GRU, LSTM, or ESN.
The micro conversation model will be trained to overfit to that specific conversation effectively
acting as a dynamic pattern encoder. The trick will be to not overfit too much so that a single
very frequent packet would over whelm the predictions, but rather map the metadata / context to
a sequence.

Some of the challenges with training a micro model on the fly include
- Compute: Since this is intended to operate on hardware limited devices, the size of the model
being trained must be tiny.
- Capacity: Since the model is so small our memory, and generalization capacity may be quite limited
- Convergence: Slow convergence is very expensive we should have a high learning rate that is directly
proportional to the model's accuracy.
- Compatability: Will be be able to accomodate new packets and novel data. We will need to employ
dropout and the dynamic learning rate to expand to new data.

@Notes:
- In this design we have the dynamically learning micro model accomapnied by some support frozen
support stucture such as the embedded, meta data mlp, output projector, and taxi layers. Training
this support architechure to generalize will be the very tricky part.
- The next iteration of this should be done with federated learning. That architechure was desiged
exactly for this sort of problem.


@TODO:
- Learning rates and dropout contingent on the loss or accuracy of the past predictions
- Freezing of everything in the model with the exception of the central GRU
- Experiments with how few parameters I can get away with
- Experiments with generalization to new conversations
- Experiments with various loss functions. What is the fastest to compute and most directly related
to predicting the next packet in the conversation?
"""
...
class OnlinePacketPredictor(nn.Module):
    """
    This generates the next packet in the conversation while also learning 'overfitting to'
    ...
    def __init__(
        ...
        self.output_projection = nn.Linear(hidden_size, VOCAB_DIM)  # Inference Freeze
...
def train_model(csv_dir: str, num_epochs=N_NUM_EPOCHS):
    ...
    for epoch in range(num_epochs):
        ...
        for i, packet in enumerate(train_loader):
            ...
            cur_conv_num = cat_features[3]
            if cur_conv_num != last_conv_num:
                # Reset for a new conversation
                print(
                    f"Moved from conversation {last_conv_num} to {cur_conv_num} after {conv_packets} packets"
                )
                conv_packets = 0
                conv_acc = 0
            ...
            packet_generator.n_conv_packets = N_MAX_CONV_PACKETS

            # Jump to the next conversation
            if conv_packets > 0 and (conv_acc / conv_packets) > N_ACC_THRESH:
                packet_generator.n_conv_packets = conv_packets
                continue
            ...
            # Stats
            train_loss += loss.item() * len(payload)
            correct = (predictions == payload).sum().item()
            train_acc += correct
            conv_acc += correct / payload.numel()
            ...
            if DEBUG_MODE:
                ...
                print_update(
                    ...
                    conv_acc=conv_acc / conv_packets,
                    conv_num=cur_conv_num,
                )
