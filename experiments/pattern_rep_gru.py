...
class AdaptivePacketGenerator(nn.Module):
    ...
    def find_matching_pattern(self, query, threshold=0.7):
        ...
        adjusted_similarities = similarities - 0.3 * usage_penalty + 0.1 * diversity_bonus

        # Get best match
        best_idx = int(torch.argmax(adjusted_similarities).item())
        best_sim = similarities[best_idx].item()  # Use original similarity for threshold
    ...
    def update_pattern_memory(self, hidden_state, match_idx=None):
        ...
        if match_idx is not None:
            ...
            pattern_change = F.cosine_similarity(old_pattern, self.pattern_memory[match_idx], dim=0)
            self.pattern_diversity[match_idx] = 0.9 * self.pattern_diversity[match_idx] + 0.1 * (1 - pattern_change)
            
            self.pattern_usage[match_idx] += 1
            self.pattern_confidence[match_idx] = min(1.0, self.pattern_confidence[match_idx] + 0.1)
            return

        # No match found, store in least diverse or least used slot
        least_used = torch.argmin(self.pattern_usage + 0.5 * self.pattern_diversity).item()
        self.pattern_memory[least_used] = hidden_state
        self.pattern_usage[least_used] = 1
        self.pattern_confidence[least_used] = 0.5
        self.pattern_diversity[least_used] = 1.0  # New patterns start with high diversity
    ...
    def forward(self, categorical, numerical, target_payload=None):
        ...
        match_idx, match_similarity = self.find_matching_pattern(self.hidden[-1].squeeze())
        ...
        if match_idx is not None and match_similarity > R_SIMILARITY_THRESH:
            ...
            noise_scale = 0.1 * (1 - match_similarity)  # More noise for stronger matches
        ...
        for t in range(seq_length):
            ...
            pos_emb = self.position_embedding(torch.tensor([t % R_MAX_SEQ_LEN], device=categorical.device))
            ...
            gating_weights = F.softmax(self.expert_gates(output.squeeze(0)), dim=-1)
            expert_outputs = torch.stack([expert(output.squeeze(0)) for expert in self.experts])
            logits = torch.sum(gating_weights.unsqueeze(-1) * expert_outputs, dim=0)

            # Anti-repetition penalty
            if t > 0:
                # Penalize recently generated tokens
                recent_penalty = torch.zeros_like(logits)
                for i in range(min(t, 5)):  # Look back 5 tokens
                    if t - i - 1 >= 0:
                        recent_token = predictions[t - i - 1]
                        recent_penalty[0, recent_token] += 0.2 * (5 - i) / 5  # Decay penalty
                logits = logits - recent_penalty
            ...
            if self.training:
                ...
                nucleus_mask = cumulative_probs <= 0.9  # Keep top 90%
        ...
        if target_payload is not None:
            ...
            for t_byte in target_payload:
                # Add positional encoding
                pos_idx = torch.tensor([self.position_counter % R_MAX_SEQ_LEN], device=categorical.device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, packet in enumerate(train_loader):
            ...
            if hidden_reliance == 0:
                n_hidden_reliant += 1
            train_hidden_rel_loss += hidden_reliance
